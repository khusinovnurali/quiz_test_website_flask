...
def admin_dashboard():
    ...
    for quiz in quizzes:
        ...
        if scores:
            ...
            completion_rate = (users_attempted / total_users * 100) if total_users else 0
        else:
           average_score = 0 
           completion_rate = 0
...
@admin_bp.route("/subject/<int:subject_id>/chapters", methods=['GET', 'POST'])
@admin_login_required
...
@admin_bp.route("/subject/<int:subject_id>/add_chapter", methods=['GET', 'POST'])
@admin_login_required
...
@admin_bp.route("/subject/<int:subject_id>/edit_chapter/<int:chapter_id>", methods=['GET', 'POST'])
@admin_login_required
...
@admin_bp.route("/subject/<int:subject_id>/delete_chapter/<int:chapter_id>", methods=['POST'])
@admin_login_required
...
@admin_bp.route("/chapter/<int:chapter_id>/quizzes", methods=['GET', 'POST'])
@admin_login_required
...
@admin_bp.route("/chapter/<int:chapter_id>/add_quiz", methods=['GET', 'POST'])
@admin_login_required
...
@admin_bp.route("/chapter/<int:chapter_id>/edit_quiz/<int:quiz_id>", methods=['GET', 'POST'])
@admin_login_required
...
@admin_bp.route("/chapter/<int:chapter_id>/delete_quiz/<int:quiz_id>", methods=['POST'])
@admin_login_required
...
def add_question(quiz_id):
    ...
    if form.validate_on_submit():
        question = Question(
            ...
            points=float(form.points.data or 1.0),
            quiz_id=quiz_id
        )
        # handle image upload
        if form.image.data:
            upload_dir = os.path.join(os.getcwd(), 'static', 'uploads', 'questions')
            ...
            try:
                ...
                pass
            question.image_path = os.path.join('static', 'uploads', 'questions', filename).replace('\\', '/')
...
def edit_question(quiz_id, question_id):
    ...
    if form.validate_on_submit():
        ...
        question.points = float(form.points.data or 1.0)
        # handle image upload
        if form.image.data:
            upload_dir = os.path.join(os.getcwd(), 'static', 'uploads', 'questions')
            os.makedirs(upload_dir, exist_ok=True)
